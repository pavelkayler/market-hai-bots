...
export function normalizeMomentumConfig(raw = {}) {
  ...
  c.tierIndices = [...new Set((Array.isArray(c.tierIndices) ? c.tierIndices : [1, 2, 3, 4, 5, 6]).map((x) => Number(x)).filter((x) => Number.isInteger(x) && x > 0 && x <= 6))].sort((a, b) => a - b);
  if (!c.tierIndices.length) c.tierIndices = [1, 2, 3, 4, 5, 6];

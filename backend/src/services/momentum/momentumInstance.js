...
export function createMomentumInstance({ id, config, marketData, sqlite, logger = console }) {
  ...
  function tryEnter(symbol, side, snap, nowMs) {
    ...
    st.pending = { side, entryPrice: snap.markPrice, placedAt: nowMs, expiresAt: nowMs + (cfg.windowMinutes * 60_000) };
  ...
  function onTick({ ts, sec }, eligibleSymbols) {
    ...
    for (const symbol of eligibleSymbols) {
      ...
      if (st.state === SYMBOL_STATE.ORDER_PENDING && st.pending) {
        if (ts > st.pending.expiresAt) {
          st.state = SYMBOL_STATE.IDLE;
          st.pending = null;
          log('ttl cancel', { symbol });
        } else {
      ...
      if ((cfg.directionMode === 'LONG' || cfg.directionMode === 'BOTH') && longOk && tryEnter(symbol, SIDE.LONG, snap, ts)) entries += 1;
      if ((cfg.directionMode === 'SHORT' || cfg.directionMode === 'BOTH') && shortOk && tryEnter(symbol, SIDE.SHORT, snap, ts)) entries += 1;
    }
  }

  ...
  return <Row className='g-3'>
    ...
    <Col md={6}><Card><Card.Body><Card.Title>Position</Card.Title>
      <Table size='sm'><thead><tr><th>Symbol</th><th>Side</th><th>Size</th><th>AvgPrice</th><th>MarkPrice</th><th>LiqPrice</th><th>PnL</th><th>SL</th><th>TP</th><th>Actions</th></tr></thead><tbody>
        <tr><td>{form.symbol}</td><td>{sideOfPosition(state.position) || '-'}</td><td>{fmt(state.position?.size || state.position?.qty)}</td><td>{fmt(state.position?.avgPrice || state.position?.entryPrice)}</td><td>{fmt(state.position?.markPrice || state.quote?.markPrice)}</td><td>{fmt(state.position?.liqPrice)}</td><td>{fmt(state.position?.unrealisedPnl || state.position?.unrealizedPnl)}</td><td>{fmt(state.position?.stopLoss)}</td><td>{Number(state.position?.takeProfit || 0) > 0 ? fmt(state.position?.takeProfit) : (tpOrder ? fmt(tpOrder.price) : '-')}</td><td><Button size='sm' variant='warning' onClick={async () => { setLastAction(await ws.request('manual.closeDemoPosition', { symbol: form.symbol })); refresh(form.symbol); }}>Закрыть позицию (Market)</Button></td></tr>
      </tbody></Table>
      ...
      <Table size='sm'><thead><tr><th>orderId</th><th>side</th><th>price</th><th>qty</th><th>leavesQty</th><th>status</th><th>reduceOnly</th><th>createdTime</th></tr></thead><tbody>
        {[...(state.orders || [])].sort((a, b) => Number(b.createdTime || 0) - Number(a.createdTime || 0)).map((o) => <tr key={o.orderId || `${o.side}-${o.price}`}><td>{shortId(o.orderId)}</td><td>{o.side}</td><td>{fmt(o.price)}</td><td>{fmt(o.qty)}</td><td>{fmt(o.leavesQty)}</td><td>{o.orderStatus || o.status || '-'}</td><td>{String(Boolean(o.reduceOnly))}</td><td>{o.createdTime ? new Date(Number(o.createdTime)).toLocaleString() : '-'}</td></tr>)}
      </tbody></Table>

...
export default function ImpulsePage() {
  ...
  const [settings, setSettings] = useState({ directionMode: 'AUTO', confirmA: true, confirmB: true });

  const onMessage = useMemo(() => (ev) => {
    let msg; try { msg = JSON.parse(ev.data); } catch { return; }
    const type = msg?.type === 'event' ? msg.topic : msg.type;
    const payload = msg?.payload;
    if (type === 'snapshot' && payload?.impulseState) setState(payload.impulseState);
    if (type === 'impulse.state' || type === 'impulse.status') setState(payload);
  ...
  return <Row className='g-3'>
    ...
    <Col md={8}><Card><Card.Body>
      ...
      <Table size='sm'><thead><tr><th>Symbol</th><th>Side</th><th>Δ15m</th><th>OI Δ15m</th><th>Time</th></tr></thead><tbody>
        {(state?.signals || []).slice(0, 30).map((s, i) => <tr key={i}><td>{s.symbol}</td><td>{s.side}</td><td>{fmtPct(s.priceDelta15m)}</td><td>{fmtPct(s.oiDelta15m)}</td><td>{fmtTs(s.ts)}</td></tr>)}
      </tbody></Table>

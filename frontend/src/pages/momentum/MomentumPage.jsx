...
const DEFAULT_FORM = {
  mode: 'demo', directionMode: 'BOTH', windowMinutes: 1, universeMode: 'SINGLE', tierIndices: [], priceThresholdPct: 0.2, oiThresholdPct: 0,
...
export default function MomentumPage() {
  ...
  useEffect(() => {
    let mounted = true;
    async function loadUniverseResult() {
    ...
    const unsub = ws.subscribe((_, parsed) => {
      if (parsed?.type !== 'event') return;
      if (parsed.topic === 'universeSearch.result') setUniverseResult(parsed.payload || null);
    });
    ws.subscribeTopics(['universeSearch.result']);
    return () => { mounted = false; unsub(); ws.unsubscribeTopics(['universeSearch.result']); };
  }, [ws]);
  ...
  const tierOptions = useMemo(() => (universeResult?.outputs?.tiers || []).map((tier) => ({
    ...
    size: Number(tier.size || tier.symbols?.length || 0),
    symbols: Array.isArray(tier.symbols) ? tier.symbols : [],
  })), [universeResult]);
  const hasTierUniverse = tierOptions.length > 0;
  const selectedTierSet = useMemo(() => new Set((form.tierIndices || []).map((x) => Number(x))), [form.tierIndices]);
  const selectedTierSymbolCount = useMemo(() => {
    const union = new Set();
    for (const tier of tierOptions) {
      if (!selectedTierSet.has(Number(tier.tierIndex))) continue;
      for (const symbol of (tier.symbols || [])) union.add(String(symbol || '').toUpperCase());
    }
    return union.size;
  }, [tierOptions, selectedTierSet]);

  const sortedSignalNotifications = useMemo(() => {
    const rows = [...(detail?.signalNotifications || [])];
    rows.sort((a, b) => {
      const sa = String(a?.symbol || '').toUpperCase();
      const sb = String(b?.symbol || '').toUpperCase();
      if (sa !== sb) return sa.localeCompare(sb);
      return Number(b?.lastUpdateTs || b?.ts || 0) - Number(a?.lastUpdateTs || a?.ts || 0);
    });
    return rows;
  }, [detail?.signalNotifications]);
  ...
  async function onStart(e) {
    ...
    const isSingle = String(nextConfig.universeMode || '').toUpperCase() === 'SINGLE';
    const tierIndices = [...new Set((nextConfig.tierIndices || []).map((x) => Number(x)).filter((x) => Number.isInteger(x) && x > 0))].sort((a, b) => a - b);
    if (isSingle && !String(nextConfig.singleSymbol || '').trim()) nextErrors.singleSymbol = 'singleSymbol is required for SINGLE mode.';
    if (!isSingle && tierIndices.length === 0) nextErrors.tierIndices = 'Select at least one tier for TIERS mode.';
    nextConfig.singleSymbol = isSingle ? String(nextConfig.singleSymbol || '').trim().toUpperCase() : '';
    nextConfig.tierIndices = isSingle ? [] : tierIndices;
    nextConfig.scanMode = isSingle ? 'SINGLE' : 'UNIVERSE';
    nextConfig.universeMode = isSingle ? 'SINGLE' : 'TIERS';
  ...
  return <Row className="g-3">
    ...
    <Col md={5}><Card><Card.Body><Card.Title>Create new bot instance</Card.Title>
      <Form>
        ...
        <Form.Group className="mb-2"><Form.Label>Universe source</Form.Label>
          <div className="border rounded p-2">
            <Form.Check
              ...
              id="mode-single"
              name="universe-mode"
              label="SINGLE (manual symbol)"
              checked={form.universeMode === 'SINGLE'}
              onChange={() => setForm({ ...form, universeMode: 'SINGLE', tierIndices: [] })}
            />
            <Form.Check
              type="radio"
              id="mode-tiers"
              name="universe-mode"
              label="TIERS (union)"
              checked={form.universeMode === 'TIERS'}
              disabled={!hasTierUniverse}
              onChange={() => setForm({ ...form, universeMode: 'TIERS', singleSymbol: '' })}
            />
            <div className="ms-3 mt-2">
              {tierOptions.map((tier) => {
                const checked = selectedTierSet.has(Number(tier.tierIndex));
                return <Form.Check
                  key={tier.tierIndex}
                  type="checkbox"
                  id={`tier-${tier.tierIndex}`}
                  label={`Tier ${tier.tierIndex} (${tier.size})`}
                  checked={checked}
                  disabled={!hasTierUniverse || form.universeMode === 'SINGLE'}
                  onChange={(e) => {
                    const next = new Set((form.tierIndices || []).map((x) => Number(x)));
                    if (e.target.checked) next.add(Number(tier.tierIndex));
                    else next.delete(Number(tier.tierIndex));
                    setForm({ ...form, universeMode: 'TIERS', singleSymbol: '', tierIndices: [...next].sort((a, b) => a - b) });
                  }}
                />;
              })}
            </div>
            <Form.Text muted className="d-block mt-1">
              {hasTierUniverse ? `Selected symbols: ${selectedTierSymbolCount}` : 'Run Universe Search first'}
            </Form.Text>
            {errors.tierIndices && <Form.Text className="text-danger d-block">{errors.tierIndices}</Form.Text>}
          </div>
        </Form.Group>
        ...
        <Form.Group className="mb-2"><Form.Label>Single symbol (USDT perp)</Form.Label>
          <Form.Control value={form.singleSymbol} placeholder="AZTECUSDT" disabled={form.universeMode !== 'SINGLE'} onChange={(e) => setForm({ ...form, singleSymbol: e.target.value.toUpperCase() })} />
          <Form.Text muted>{form.universeMode === 'SINGLE' ? 'Required in SINGLE mode.' : 'Disabled when Tier mode is selected.'}</Form.Text>
        ...
        <div className="d-flex gap-2">
          <Button type="button" onClick={onStart} disabled={form.universeMode === 'TIERS' && !hasTierUniverse}>Start</Button>
        </div>
      </Form>
    </Card.Body></Card></Col>
    ...
    <Col md={12}><Card><Card.Body><Card.Title>Selected instance details</Card.Title>
      ...
      {detail && <div>Open positions: {detail.openPositions?.length || 0} | Pending triggers: {detail.pendingOrders?.length || 0} | Universe: {detail?.config?.scanMode === 'SINGLE' ? `SINGLE: ${detail?.config?.singleSymbol || '-'}` : `TIERS: ${(detail?.config?.tierIndices || []).join(',') || '-'} (union=${Number(detail?.config?.resolvedSymbolsCount || 0)})`} | W: {detail?.config?.windowMinutes}m | Hedge: {detail?.hedgeMode || 'UNKNOWN'} | Margin desired: {detail?.marginModeDesired || 'ISOLATED'} | Isolated preflight: {detail?.isolatedPreflightOk ? 'OK' : (detail?.isolatedPreflightError || 'N/A')} | Trades: {detail?.stats?.trades || 0} | Wins: {detail?.stats?.wins || 0} | Losses: {detail?.stats?.losses || 0} | Winrate: {(detail?.stats?.trades ? ((detail.stats.wins / detail.stats.trades) * 100) : 0).toFixed(1)}% | PnL: {Number(detail?.stats?.pnl || 0).toFixed(2)} | Fees: {Number(detail?.stats?.fees || 0).toFixed(2)}</div>}

      {detail && <><h6>Open Orders / Pending Triggers</h6><Table size="sm" className="mt-2"><thead><tr><th>Symbol</th><th>State</th><th>Side</th><th>Trigger/Entry</th><th>Current</th><th>TP/SL</th><th>TP/SL Status</th><th>Reason</th><th>Created</th><th>Age</th><th>Actions</th></tr></thead><tbody>
        {(detail.pendingOrders || []).map((p) => <tr key={`pending_${p.symbol}`}><td>{p.symbol}</td><td>{p.state || 'TRIGGER_PENDING'}</td><td>{p.side || '-'}</td><td>{p.triggerPrice || '-'}</td><td>{Number(p.currentPrice || 0).toFixed(6)}</td><td>-</td><td>{p.state === 'FAILED' ? 'FAILED' : '-'}</td><td>{p.state === 'FAILED' ? `${p.failReason || 'unknown'}${p.failDetail ? `: ${p.failDetail}` : ''}` : '-'}</td><td>{p.createdAtMs ? new Date(p.createdAtMs).toLocaleTimeString() : '-'}</td><td>{Number(p.ageSec || 0)}s</td><td>{p.state === 'FAILED' ? <Button size="sm" variant="outline-secondary" onClick={() => onCancelEntry(p.symbol)}>Clear</Button> : <Button size="sm" variant="outline-warning" onClick={() => onCancelEntry(p.symbol)}>Cancel entry</Button>}</td></tr>)}
        {(detail.openPositions || []).map((p) => <tr key={`pos_${p.symbol}`}><td>{p.symbol}</td><td>IN_POSITION</td><td>{p.side}</td><td>{p.entryPriceActual || p.entryPrice}</td><td>{Number(p.currentPrice || 0).toFixed(6)}</td><td><div>TP {Number(p.tpRoiPct ?? detail?.config?.tpRoiPct ?? 0)}% / SL {Number(p.slRoiPct ?? detail?.config?.slRoiPct ?? 0)}%</div><div className="text-muted" style={{ fontSize: 11 }}>{p.tpPrice} / {p.slPrice}</div></td><td>{p.tpSlStatus || 'PENDING'}</td><td>{p.tpSlStatus === 'FAILED' ? (p.tpSlFailReason || 'unknown') : '-'}</td><td>-</td><td>-</td><td>-</td></tr>)}
      </tbody></Table></>}
      ...
      <div style={{ maxHeight: 360, overflowY: 'auto', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, marginBottom: 12 }}>
        <Table size="sm" className="mb-0">
          <thead><tr><th>Symbol</th><th>Side</th><th>Action</th><th>PriceΔ%</th><th>OIΔ%</th><th>Turnover gate</th><th>Hold</th><th>Message</th><th>Last Update (ms)</th></tr></thead>
          <tbody>
            {sortedSignalNotifications.slice(0, 100).map((n, idx) => <tr key={`${n.ts}_${n.symbol || 'NA'}_${idx}`}>
              <td>{n.symbol || '-'}</td>
              <td>{n.side || '-'}</td>
              ...
              <td>{Math.max(0, Math.round(Date.now() - Number(n.lastUpdateTs || n.ts || 0)))}</td>
            </tr>)}
